...
def display_topic(universe):
    ...
    with col2:
        ...
        with refresh_col:
            ...
            st.write("")
            st.button("🔄", help="Refresh data", on_click=refresh_data)

        with info_col:
            topic_data = (
                all_feed_data[all_feed_data["topic"] == selected_topic] if all_feed_data is not None else pd.DataFrame()
            )
            data_points = []
            ...
            for feature in features:
                ...
                try:
                    ...
                    if feature_fig is not None:
                        ...
                        st.info(f"No {feature} data available for {topic_display_name(selected_topic)} from {source}")
                except Exception as e:
